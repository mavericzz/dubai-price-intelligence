...
// ---------------------------------------------------------------------------
// Off-plan specific queries
// ---------------------------------------------------------------------------

export async function getOffPlanListings(
  filters: Omit<ListingFilters, 'is_off_plan'>,
  sort: { field: ListingSortField; direction: SortDirection },
  page: PaginationParams,
): Promise<OffPlanListing[]> {
  let query = supabase.from('listings').select('*').eq('is_off_plan', true);
  if (filters.area !== undefined) query = query.eq('area', filters.area);
  if (filters.property_type !== undefined) query = query.eq('property_type', filters.property_type);
  if (filters.beds !== undefined) query = query.eq('beds', filters.beds);
  if (filters.is_active !== undefined) query = query.eq('is_active', filters.is_active);
  ...
  if (filters.min_drop_percent !== undefined) {
    const min = filters.min_drop_percent;
    computed = computed.filter((l) => l.drop_percent !== null && l.drop_percent >= min);
  }
  if (filters.min_yield !== undefined) {
    const min = filters.min_yield;
    computed = computed.filter((l) => l.estimated_gross_yield !== null && l.estimated_gross_yield >= min);
  }
  if (filters.motivation !== undefined) {
    const motivation = filters.motivation;
    computed = computed.filter((l) => l.motivation_score === motivation);
  ...
  const offset = (page.page - 1) * page.limit;
  const paged = computed.slice(offset, offset + page.limit);

  if (paged.length === 0) return [];

  // Batch-fetch price history ordered ascending so the first row = launch price
  ...
  type PhRow = { listing_id: string; price: number; recorded_at: string };
  const firstPriceByListing = new Map<string, number>();
  for (const row of (phData ?? []) as PhRow[]) {
    if (!firstPriceByListing.has(row.listing_id)) {
      firstPriceByListing.set(row.listing_id, row.price);
    }
  }

  return paged.map((listing) => {
    const launchPrice = firstPriceByListing.get(listing.id) ?? null;
    const dropSinceLaunch =
      launchPrice !== null && listing.price !== null && launchPrice > 0
        ? ((launchPrice - listing.price) / launchPrice) * 100
        : null;
    return { ...listing, launch_price: launchPrice, drop_since_launch: dropSinceLaunch };
  });
}
...
export async function getOffPlanAreas(): Promise<string[]> {
  ...
  const areas = [
    ...new Set(
      ((data ?? []) as AreaRow[]).map((r) => r.area).filter((a): a is string => a !== null),
    ),
  ];
  return areas.sort();
}
